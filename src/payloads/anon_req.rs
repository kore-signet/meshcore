...
pub struct AnonymousRequest<'a, P>
where
...
impl<'a, P: SerDeser + Encryptable> AnonymousRequest<'a, P> {
    pub fn mac<H: HmacImpl>(&self, mac_key: &[u8]) -> [u8; 32] {
        H::mac(&self.ciphertext, mac_key)
    }
}

impl<'a, P: Encryptable> PacketPayload for AnonymousRequest<'a, P> {
    const PAYLOAD_TYPE: PayloadType = PayloadType::AnonReq;
}

impl<'a, P: SerDeser + Encryptable> SerDeser for AnonymousRequest<'a, P> {
    type Representation<'data> = AnonymousRequest<'data, P>;

    fn encode_size<'data>(object: &Self::Representation<'data>) -> usize {
        1 + 32 + 2 + object.ciphertext.len()
    }
    ...
    fn encode<'data, 'out>(
        ...
        out.write_slice(&object.sender_key);
...
impl<'a, P: SerDeser + Encryptable> ContainsEncryptable for AnonymousRequest<'a, P> {
    type Output = P;
